...
/**
  * This provides an event sourced behavior. It has a state, [[LagomtrainingState]], which
  ...
  */
object LagomtrainingBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[LagomtrainingCommand]): Behavior[LagomtrainingCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, LagomtrainingEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[LagomtrainingCommand, LagomtrainingEvent, LagomtrainingState](
        persistenceId = persistenceId,
        emptyState = LagomtrainingState.initial,
...
/**
  ...
  */
case class LagomtrainingState(message: String, timestamp: String) {
  def applyCommand(cmd: LagomtrainingCommand): ReplyEffect[LagomtrainingEvent, LagomtrainingState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: LagomtrainingEvent): LagomtrainingState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[LagomtrainingEvent, LagomtrainingState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[LagomtrainingEvent, LagomtrainingState] =
    Effect
...
object LagomtrainingState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: LagomtrainingState = LagomtrainingState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[LagomtrainingCommand]("LagomtrainingAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[LagomtrainingState] = Json.format
}

/**
  * This interface defines all the events that the LagomtrainingAggregate supports.
  */
sealed trait LagomtrainingEvent extends AggregateEvent[LagomtrainingEvent] {
  def aggregateTag: AggregateEventTag[LagomtrainingEvent] = LagomtrainingEvent.Tag
}

object LagomtrainingEvent {
  val Tag: AggregateEventTag[LagomtrainingEvent] = AggregateEventTag[LagomtrainingEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends LagomtrainingEvent
...
/**
  ...
  */
trait LagomtrainingCommandSerializable

/**
  * This interface defines all the commands that the LagomtrainingAggregate supports.
  */
sealed trait LagomtrainingCommand
    extends LagomtrainingCommandSerializable
...
/**
  ...
  * application loader.
  */
object LagomtrainingSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[LagomtrainingState],
